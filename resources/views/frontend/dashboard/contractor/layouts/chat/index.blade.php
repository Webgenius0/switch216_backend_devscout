...
@push('scripts')
    <script>
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    res.data.forEach(function(chatRoom) {
                        ...
                        let avatarUrl = chatRoom.customer.avatar ? "{{ asset('') }}" + chatRoom
                            .customer
                            .avatar : "{{ asset('backend/assets/images/avatar_defult.png') }}";

                        $('#chatRoomList').append(`
                            <div class="item" onclick="getChatRoom(${chatRoom.chatRoom_id})">
                                <div class="profile-container">
                                    <img src="${avatarUrl}" alt="user" />
                                </div>
                                <div class="message-container">
                                    <div class="message-container-top">
                                        <div class="title">${chatRoom.customer.name}</div>
                                        <div class="time">${chatRoom.last_message ? (chatRoom.last_message.created_at || '') : ''}</div>
                                    </div>
                                    <div class="text">
                                ...
                                ${chatRoom.unread_count ? `<div class="chat-count">${chatRoom.unread_count}</div>` : ""}
                            </div>
        ...
        let Opnestoreurl = "{{ route('contractor.message.send_message', ':chatRoomId') }}".replace(
            ':chatRoomId', {{ $lastChatRommMessage->id }});
